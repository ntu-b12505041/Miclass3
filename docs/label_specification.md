...
MI SCP statement + Stadium III                 -> excluded
...
For the primary protocol, LBBB is positive only when `scp_codes` contains a positive LBBB code. Modified Sgarbossa is then positive if any of: concordant STE >=0.1 mV; concordant STD >=0.1 mV in V1-V3; or discordant STE >=0.1 mV with ST/S >=0.25. The simplified raw-waveform LBBB rule is retained only for sensitivity analyses. Keep the raw measurements, beat counts, baseline source, and reviewer status in the morphology CSV.
...
- Exclude `Stadium III`, the completed/old infarction category. Retain `Stadium II-III` in the primary cohort because it spans an ambiguous subacute-to-chronic transition; a stricter exclusion is a future sensitivity analysis.
- Do not use `lbbb` or `modified_sgarbossa_positive` as primary classifier inputs; they are direct proxy-label routing flags. The LBBB auxiliary target may be supplied separately to a waveform-only auxiliary head.
