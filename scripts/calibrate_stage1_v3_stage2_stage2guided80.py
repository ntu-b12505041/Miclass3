...
from miclass3.stage2_guided_gate import stage2_guided_predict, search_stage2_guided_gate
...
def load_joined(stage1_path: Path, stage2_path: Path) -> pd.DataFrame:
    ...
    _require(s1, ["ecg_id", "actual_id", "p_mi"], stage1_path)
    _require(s2, ["ecg_id", "actual_id", "p_stemi_given_mi"], stage2_path)

    left = s1[["ecg_id", "actual_id", "p_mi"]].copy()
    ...
    if (joined["actual_id_s1"] != joined["actual_id_s2"]).any():
        raise ValueError("Stage1/Stage2 actual labels do not match")
    return joined.rename(columns={"actual_id_s1": "actual_id"}).drop(
...
def save_predictions(
    ...
    *,
    ...
    out["rescued_by_stage2"] = rescue.astype(int)
    out["mi_threshold"] = mi_threshold
    out["stemi_threshold"] = stemi_threshold
    out["rescue_floor"] = rescue_floor
    out["rescue_stemi_max"] = rescue_stemi_max
    out.to_csv(path, index=False)


def rescue_counts(joined: pd.DataFrame, rescue: np.ndarray) -> dict[str, int]:
    y = joined["actual_id"].to_numpy(dtype=int)
    return {
        "total": int(rescue.sum()),
        "non_mi": int(np.sum(rescue & (y == 0))),
        "stemi_proxy": int(np.sum(rescue & (y == 1))),
        "nstemi_proxy": int(np.sum(rescue & (y == 2))),
    }
...
def run_validation(cfg: dict, config_path: str) -> None:
    ...
    best, rows = search_stage2_guided_gate(
    ...
    save_predictions(
        ...
        rescue_stemi_max=best.rescue_stemi_max,
        path=out_dir / "validation_predictions.csv",
    )

    counts = rescue_counts(joined, best.rescue_mask)
    ...
    summary = {
        "experiment": "stage1_v3_stage2_stage2_guided_nstemi_rescue",
        "config": config_path,
        ...
        "test": None,
        "note": "All thresholds and rescue parameters are selected on Fold 9 only.",
    }
    ...
    print(f"Selected Stage2 rescue max P(STEMI|MI): {best.rescue_stemi_max:.6f}")
    print(
        "Rescued validation records: "
        f"total={counts['total']} nonMI={counts['non_mi']} "
        f"STEMI={counts['stemi_proxy']} NSTEMI={counts['nstemi_proxy']}"
    )
    print_result("VALIDATION STAGE2-GUIDED NSTEMI RESCUE", best.summary, best.confusion_matrix)
...
def run_finalize(cfg: dict) -> None:
    ...
    pred, rescue = stage2_guided_predict(
        ...
        rescue_stemi_max=float(val["rescue_stemi_max"]),
    )
    ...
    test["rescue_counts"] = rescue_counts(joined, rescue)
    summary["test"] = test
    ...
    print_result("FROZEN STAGE2-GUIDED RESCUE -> FOLD 10", test_summary, cm)
    counts = test["rescue_counts"]
    print(
        "Rescued test records: "
        f"total={counts['total']} nonMI={counts['non_mi']} "
        f"STEMI={counts['stemi_proxy']} NSTEMI={counts['nstemi_proxy']}"
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--config", default="configs/stage1_v3_stage2_stage2guided80.yaml"
    )
