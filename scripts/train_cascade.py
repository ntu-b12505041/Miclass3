...
from miclass3.models import SEResNet
...
def target_for_stage(labels, stage: int):
    if stage == 1:
        return (labels != 0).long()  # MI = STEMI-proxy or NSTEMI-proxy
    if stage == 2:
        # Stage-2 datasets contain only MI labels: 1=STEMI-proxy, 2=NSTEMI-proxy.
...
def predict_binary(model, loader, device, stage: int):
    ...
    with torch.no_grad():
        for x, y, _ in loader:
            logits = model(x.to(device))["class_logits"]
...
def train_stage(stage: int, train_loader, val_loader, device, cfg: dict):
    ...
    # Count the binary labels from the underlying stage-specific training frame.
    ...
    weights = normalized_class_weights(counts, float(training.get("class_weight_power", 0.5)))
    ...
    for epoch in range(1, int(training["epochs"]) + 1):
        ...
        for x, y, _ in train_loader:
            logits = model(x.to(device))["class_logits"]
            target = target_for_stage(y.to(device), stage)
    ...
    return model, history, {"binary_class_counts": counts.tolist(), "class_weights": weights.tolist()}
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train a literal MI -> STEMI/NSTEMI two-stage ECG cascade.")
    ...
    stage1_datasets = {name: dataset(frame) for name, frame in full_frames.items()}
    stage2_datasets = {name: dataset(frame) for name, frame in stage2_frames.items()}
    stage1_loaders = {
        name: DataLoader(ds, batch_size=batch_size, shuffle=(name == "train"), num_workers=workers)
    ...
    print("Training stage 2: STEMI-proxy vs NSTEMI-proxy on MI-only records")
    ...
    # Stage-specific reporting and final three-class cascade reporting.
    ...
    for split in ("train", "val", "test"):
        ...
        # Stage 2 is reported on the true MI subset, which is the population it
        ...
        # For final routing we score all records with the conditional model;
        # non-MI records are ignored by the hard gate when stage 1 rejects MI.
        full_stage2_loader = DataLoader(stage1_datasets[split], batch_size=batch_size, shuffle=False, num_workers=workers)
        _, p_stemi_given_mi = predict_binary(stage2, full_stage2_loader, device, stage=2)
    ...
    torch.save(
        ...
        out_dir / "stage2_stemi_vs_nstemi.pt",
    )
